				 ...
				 string course_name)
{
	...
	for (int i = 0; i < size; i++)
	{
		char course = student[ordered_index[i]].course;
...
void create_array(Student student[], string text, int size)
{
	int j = 0;
	...
	for (int idx = 0; idx < size; idx++)
	{
		...
		while (text[j] != '\n')
		{
			if (text[j] != ',')
			{
				if (count == 0)
				{
					student[idx].last_name[word_idx] = text[j];
					word_idx++;
				}

				else if (count == 1)
				{
					student[idx].first_name[word_idx] = text[j];
					word_idx++;
				}

				else if (count == 2)
				{
					student[idx].course = text[j];
				}

				else if (count == 3)
				{
					student[idx].test1_score[word_idx] = text[j];
					word_idx++;
				}

				else if (count == 4)
				{
					student[idx].test2_score[word_idx] = text[j];
					word_idx++;
				}

				else if (count == 5)
				{
					student[idx].final_score[word_idx] = text[j];
					word_idx++;
				}
			}
			else
...
int main()
{
	...
	create_array(student, text, size);
	...
	cout << "---------------------" << endl;
	cout << endl << endl;
	print_array(student, ordered_index, size, 'E', "ENGLISH CLASS");
	print_array(student, ordered_index, size, 'H', "HISTORY CLASS");
	print_array(student, ordered_index, size, 'M', "MATH CLASS");
